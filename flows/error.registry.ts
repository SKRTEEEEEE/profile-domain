...
export function createDomainError(
  ...
  location: Function,
  func: string,
  friendlyDesc?: IntlBase | "tryAgainOrContact" | "d" | "credentials" | "credentials--mock",
  meta?: ErrorMeta,
  friendlyHeader?: "ups"
): DomainError {
  const Ctor = ERROR_CLASS_REGISTRY[code];
  if(friendlyDesc === "tryAgainOrContact"){
    return new Ctor(location, func, {
    ...
    return new Ctor(location, func, friendlyDesc, Object.assign({}, meta, {
      desc: {
        es: 'Ups, ha ocurrido un error',
        en: 'Oops, an error occurred',
        ca: 'Ups, ha ocorregut un error',
